...
public class UpdateManager {
    ...
    public void checkThisVersionTable(Context context) {
        ...
        userList = userDao.query(new LoginTable());
    ...
    public void startUpdateDb(Context context) {
        ...
        if (getLocalVersionInfo()) {
            ...
            try {
                //更新每个用户的数据库
                for (LoginTable user : userList) {
                    String logicDbDir =
                            parentFile.getAbsolutePath() + "/update" + "/" + user.getUser_id() +
                                    "/logic.db";

                    String logicCopy = bakFile.getAbsolutePath() + "/" + user.getUser_id() +
                            "/logic.db";
